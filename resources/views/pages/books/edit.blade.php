<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            <a href="{{ route('book.index') }}" class="font-normal cursor-pointer hover:underline">{{ __('Books') }}</a> / <span class="font-normal">{{ __('Edit') }} </span>/ {{ $book->title }}
        </h2>
    </x-slot>
            ...
            <b>Cover</b> - Make sure the cover matches the Volume Number <small>(if applicable)</small> and the Language & Publisher. Get the highest quality possible thats under 2mb in filesize.<br><br>">
                <x-slot name="content">
                    ...
                    <form action="{{ route('book.update', ['book' => $book->id]) }}" method="POST" enctype="multipart/form-data">
                        ...
                        @method('PUT')
                        <div class="">
                            <x-jet-label for="title" value="{{ __('Original Title') }}" />
                            <p>{{$book->title}}</p>
                            <input name="title" value="{{$book->title}}" type="hidden" />
                        </div>
                        <div class="">
                            <x-jet-label for="clean_title" value="{{ __('Clean Title') }}" />
                            <x-jet-input required value="{{$book->clean_title}}" name="clean_title" placeholder="E.g. Monkey High, Vol. 6" type="text" class="mt-1 block w-full" autocomplete="clean_title" />
                            <x-jet-input-error for="clean_title" class="mt-2" />
                        </div>
                        ...
                        <div class="mt-2 flex">
                            ...
                            <div class="w-1/3 ml-1">
                                ...
                                <x-jet-input value="{{$book->volume_number}}" name="volume_number" placeholder="E.g. 10" type="text" class="mt-1 block w-full" autocomplete="volume_number" />
