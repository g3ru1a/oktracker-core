    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            <a href="{{ route('book.index') }}"
                class="font-normal cursor-pointer hover:underline">{{ __('Books') }}</a> / {{ __('Create') }}
        </h2>
    </x-slot>
                ...
                <x-slot name="content">
                    ...
                    <form action="{{ route('book.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="">
                            ...
                            <x-jet-input required name="title" placeholder="E.g. Monkey High, Vol. 6" type="text"
                        ...
                        <div class="mt-2 flex">
                            <div class="w-1/2 mr-1">
                                ...
                                <x-jet-input required name="isbn_10" placeholder="E.g. 1642731447" type="text"
                                    class="mt-1 block w-full" autocomplete="isbn_10" />
                                <x-jet-input-error for="isbn_10" class="mt-2" />
                            </div>
                            <div class="w-1/2 ml-1">
                                <x-jet-label for="isbn_13" value="{{ __('ISBN 13') }}" />
                                <x-jet-input required name="isbn_13" placeholder="E.g. 9781642731446" type="text"
                                    class="mt-1 block w-full" autocomplete="isbn_13" />
                        ...
                        <div class="mt-2 flex">
                            <div class="w-1/3 mr-1">
                                <x-jet-label for="publish_date"
                                    value="{{ __('Publish Date') }} ({{ __('Optional') }})" />
                                <x-jet-input name="publish_date" placeholder="E.g. Seiso, Mark" type="date"
                                    class="mt-1 block w-full" autocomplete="publish_date" />
                                <x-jet-input-error for="publish_date" class="mt-2" />
                            </div>
                            <div class="w-1/3 ml-1">
                                <x-jet-label for="pages" value="{{ __('Pages') }} ({{ __('Optional') }})" />
                                <x-jet-input name="pages" placeholder="E.g. 180" type="number" class="mt-1 block w-full"
                                    autocomplete="pages" />
                                <x-jet-input-error for="pages" class="mt-2" />
                            </div>
                            <div class="w-1/3 ml-1">
                                <x-jet-label for="volume_number"
                                    value="{{ __('Volume Number') }} ({{ __('Optional') }})" />
                                <x-jet-input name="volume_number" placeholder="E.g. 10" type="number"
                                    class="mt-1 block w-full" autocomplete="volume_number" />
                        ...
                        <div class="mt-2 flex">
                            <div class="w-full ml-1">
                                ...
                                <label class="inline-flex items-center mt-3">
                                    <input name="is_oneshot" type="checkbox"
                                        class="form-checkbox h-5 w-5 text-gray-600"><span
                                        class="ml-2 text-gray-700">This is the only book in the series</span>
                                </label>
                            </div>
                        </div>
